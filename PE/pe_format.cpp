...
bool PE::get_section_header( const uint32_t module_base )
{
	...
	if( m_dos_header )
	{
		...
		m_section_header       = reinterpret_cast< uint32_t >( m_nt_header ) + 0x18 + m_size_optional_header;
...
bool PE::set_section_text( const uint32_t module_base )
{
	if( get_section_header( module_base ) )
	{
		for( short index{ 0 }; index < m_num_of_sections; ++index )
		{
			...
			const auto name                  { read_remote_str( reinterpret_cast< uint32_t >( section_header ), 8 ) };

			if( name != ".text" )
				continue;

			const auto virtual_address       { read_remote< uint32_t >( reinterpret_cast< uint32_t >( &section_header->Misc.VirtualSize ) ) };
			const auto raw_data              { read_remote< uint32_t >( reinterpret_cast< uint32_t >( &section_header->SizeOfRawData ) ) };
			...
			m_section_text.m_page_size       = ( virtual_address > raw_data ? virtual_address : raw_data );
